...
final class PostsRelatedTest extends IntegrationTestCase
{
    ...
    public function testSearchEndpointExcludesSelfAndFiltersByQuery(): void
    {
        ...
        $this->client->request(HttpMethodEnum::Get->value, $this->urlGenerator->generate('backend_editorial_posts_search', ['q' => 'apple', 'excludeId' => $first->getId()]));
    ...
    public function testSearchEndpointResolvesByIds(): void
    {
        ...
        $this->client->request(HttpMethodEnum::Get->value, $this->urlGenerator->generate('backend_editorial_posts_search', ['ids' => sprintf('%d,%d', $first->getId(), $third->getId())]));
        self::assertSame(200, $this->client->getResponse()->getStatusCode());
        $body = json_decode((string) $this->client->getResponse()->getContent(), true);
