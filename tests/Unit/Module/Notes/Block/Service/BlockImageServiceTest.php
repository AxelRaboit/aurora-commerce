<?php

declare(strict_types=1);

namespace Aurora\Tests\Unit\Module\Notes\Block\Service;

use Aurora\Core\Media\Library\Enum\MimeTypeEnum;
use Aurora\Core\User\Entity\User;
use Aurora\Module\Notes\Block\Service\BlockImageService;
use PHPUnit\Framework\TestCase;
use ReflectionProperty;
use RuntimeException;
use Symfony\Component\Filesystem\Filesystem;
use Symfony\Component\HttpFoundation\File\Exception\FileException;
use Symfony\Component\HttpFoundation\File\UploadedFile;

use const DIRECTORY_SEPARATOR;
use const FILE_APPEND;

final class BlockImageServiceTest extends TestCase
{
    private string $storageDir;
    private BlockImageService $imageService;
    private Filesystem $filesystem;

    protected function setUp(): void
    {
        $this->storageDir = sys_get_temp_dir().'/aurora-block-images-'.bin2hex(random_bytes(4));
        $this->filesystem = new Filesystem();
        $this->imageService = new BlockImageService($this->storageDir, $this->filesystem);
    }

    protected function tearDown(): void
    {
        if (is_dir($this->storageDir)) {
            $this->filesystem->remove($this->storageDir);
        }
    }

    public function testStoreValidPngWritesUnderUserDirWithUuidName(): void
    {
        $user = $this->makeUser(7);
        $upload = $this->makeUpload(MimeTypeEnum::Png, 64);

        $filename = $this->imageService->store($upload, $user);

        self::assertMatchesRegularExpression('/^[0-9a-f-]{36}\.png$/', $filename);
        self::assertFileExists($this->storageDir.'/7/'.$filename);
    }

    public function testStoreRejectsUnsupportedMime(): void
    {
        $this->expectException(FileException::class);
        $this->imageService->store($this->makeUpload(null, 64), $this->makeUser());
    }

    public function testStoreRejectsOversizedFile(): void
    {
        $this->expectException(FileException::class);
        $oversize = BlockImageService::MAX_FILE_SIZE + 1;
        $this->imageService->store($this->makeUpload(MimeTypeEnum::Png, $oversize), $this->makeUser());
    }

    public function testPathReturnsAbsolutePathOfUserFile(): void
    {
        $user = $this->makeUser(9);
        $filename = $this->imageService->store($this->makeUpload(MimeTypeEnum::Jpeg, 32), $user);

        $resolved = $this->imageService->path($filename, $user);

        self::assertFileExists($resolved);
        self::assertStringStartsWith($this->storageDir.DIRECTORY_SEPARATOR.'9', $resolved);
    }

    public function testPathThrowsOnMissingFile(): void
    {
        $this->expectException(RuntimeException::class);
        $this->imageService->path('does-not-exist.png', $this->makeUser());
    }

    public function testPathRefusesTraversalAttempt(): void
    {
        $user = $this->makeUser(1);
        // Plant a file in user 2's dir, then try to read it as user 1.
        $other = $this->makeUser(2);
        $this->imageService->store($this->makeUpload(MimeTypeEnum::Png, 32), $other);
        $otherFiles = glob($this->storageDir.'/2/*.png');
        self::assertNotEmpty($otherFiles);
        $bareName = basename($otherFiles[0]);

        $this->expectException(RuntimeException::class);
        $this->imageService->path('../2/'.$bareName, $user);
    }

    public function testDeleteRemovesFileAndIsIdempotent(): void
    {
        $user = $this->makeUser(3);
        $filename = $this->imageService->store($this->makeUpload(MimeTypeEnum::Webp, 32), $user);

        $this->imageService->delete($filename, $user);
        self::assertFileDoesNotExist($this->storageDir.'/3/'.$filename);

        // Second call must not throw — orphan cleanup runs multiple times
        // (the Manager's diff hook may be triggered twice in a session).
        $this->imageService->delete($filename, $user);
        self::assertTrue(true);
    }

    public function testDeleteOnUnknownFilenameIsNoop(): void
    {
        // Called with a filename that was never stored — must not raise.
        $this->imageService->delete('phantom.png', $this->makeUser(5));
        self::assertTrue(true);
    }

    private function makeUser(int $id = 42): User
    {
        $user = new User();
        (new ReflectionProperty(User::class, 'id'))->setValue($user, $id);

        return $user;
    }

    /**
     * Build a real, libmagic-recognisable fixture file for the given MIME
     * via GD, then pad it to the requested size. `null` produces a non-
     * image (`application/pdf`) used by the rejection test.
     */
    private function makeUpload(?MimeTypeEnum $imageMime, int $size): UploadedFile
    {
        $mime = $imageMime?->value ?? 'application/pdf';
        $extension = $imageMime?->extension() ?? 'bin';
        $path = tempnam(sys_get_temp_dir(), 'aurora-block-upload-');

        $image = imagecreatetruecolor(1, 1);
        match ($imageMime) {
            MimeTypeEnum::Png => imagepng($image, $path),
            MimeTypeEnum::Jpeg, MimeTypeEnum::Jpg => imagejpeg($image, $path),
            MimeTypeEnum::Webp => imagewebp($image, $path),
            MimeTypeEnum::Gif => imagegif($image, $path),
            default => file_put_contents($path, "%PDF-1.4\n%fake pdf\n"),
        };
        imagedestroy($image);

        $currentSize = filesize($path);
        if ($size > $currentSize) {
            file_put_contents($path, str_repeat('x', $size - $currentSize), FILE_APPEND);
        }

        return new UploadedFile(
            path: $path,
            originalName: 'screenshot.'.$extension,
            mimeType: $mime,
            test: true,
        );
    }
}
