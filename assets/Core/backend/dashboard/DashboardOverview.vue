...
const MODULES = [
    { id: "editorial",  label: () => t("backend.nav.sections.editorial"),  icon: FileText },
    { id: "crm",        label: () => t("backend.nav.sections.crm"),        icon: Users },
    { id: "erp",        label: () => t("backend.nav.sections.erp"),        icon: Package },
    { id: "billing",    label: () => t("backend.nav.sections.billing"),    icon: Receipt },
    { id: "ecommerce",  label: () => t("backend.nav.sections.ecommerce"),  icon: ShoppingCart },
    { id: "photo",      label: () => t("backend.nav.sections.photo"),      icon: Camera },
];
</script>
    ...
    <div class="space-y-6">
        ...
        <section v-show="activeModule === 'editorial'" class="space-y-4">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-surface border border-line rounded-xl p-4">
                    <div class="flex items-center justify-between mb-3">
                        <span class="text-xs font-medium text-secondary uppercase tracking-wide">{{ t('backend.stats.menus') }}</span>
            ...
            <div v-if="stats.recentPosts?.length" class="bg-surface border border-line/60 rounded-xl p-5">
                ...
                <div class="space-y-3">
                    <div v-for="post in stats.recentPosts" :key="post.id" class="flex items-center justify-between gap-3 text-sm">
                        ...
                        <span class="px-2 py-0.5 text-xs rounded-md" :class="statusBadge(post.status)">{{ t(`backend.stats.postStatus.${post.status}`, post.status) }}</span>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section v-show="activeModule === 'erp'" class="space-y-4">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-surface border border-line rounded-xl p-4">
                    <div class="flex items-center justify-between mb-3">
                        <span class="text-xs font-medium text-secondary uppercase tracking-wide">{{ t('backend.erp.products.status.active') }}</span>
                        <div class="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center">
        ...
        <section v-show="activeModule === 'billing'" class="space-y-4">
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div class="bg-surface border border-line rounded-xl p-4">
                    <div class="flex items-center justify-between mb-3">
                        <span class="text-xs font-medium text-secondary uppercase tracking-wide">{{ t('backend.nav.ocr_import') }}</span>
                        <div class="w-8 h-8 rounded-lg bg-sky-500/10 flex items-center justify-center">
        ...
        <section v-show="activeModule === 'ecommerce'" class="space-y-4">
            ...
            <div v-if="hasOrders" class="bg-surface border border-line/60 rounded-xl p-5">
                <h3 class="text-sm font-semibold text-primary mb-4">{{ t('backend.nav.orders') }}</h3>
                <div class="h-64">
