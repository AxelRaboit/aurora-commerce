...
<template>
    <div class="text-center">
        ...
        <h1 class="text-2xl font-bold text-primary mb-3">{{ t('auth.register_confirm.heading') }}</h1>
        <p class="text-secondary text-sm leading-relaxed mb-8">{{ t('auth.register_confirm.message') }}</p>

        <AppLink :href="loginPath" class="text-sm">{{ t('auth.register_confirm.login_link') }}</AppLink>

        <div v-if="pendingEmail" class="mt-8 pt-8 border-t border-line/40">
            <p v-if="resent" class="text-sm text-emerald-400">{{ t('auth.register_confirm.resent_success') }}</p>
            <template v-else>
                <p class="text-sm text-secondary mb-3">{{ t('auth.register_confirm.resend_hint') }}</p>
                <form method="POST" :action="resendPath">
                    <button type="submit" class="text-sm text-indigo-400 hover:underline font-medium">
                        {{ t('auth.register_confirm.resend_link') }}
                    </button>
                </form>
            </template>
        </div>
