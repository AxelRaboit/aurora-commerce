...
async function deleteField(field) {
    ...
    if (!confirm(t("admin.forms.deleteFieldConfirm", { label: fieldLabel(field) }))) return;
...
<template>
            ...
            <div v-if="activeTab === 'fields'" class="p-5 flex flex-col gap-4 overflow-y-auto flex-1">
                ...
                >
                    ...
                    >
                        ...
                        <AppIconButton color="indigo" :title="t('common.edit')" v-on:click="openEditField(field)">
                            <Pencil class="w-4 h-4" :stroke-width="2" />
                        </AppIconButton>
                        <AppIconButton color="rose" :title="t('common.delete')" v-on:click="deleteField(field)">
                            <Trash2 class="w-4 h-4" :stroke-width="2" />
                        </AppIconButton>
                    </div>
                </VueDraggable>
    ...
    <AppModal :show="showFieldModal" max-width="sm" v-on:close="showFieldModal = false">
        ...
        <div class="flex justify-end gap-2 pt-2 border-t border-line">
            <AppButton variant="ghost" size="md" v-on:click="showFieldModal = false">{{ t("common.cancel") }}</AppButton>
            <AppButton variant="primary" size="md" :disabled="fieldSaving" v-on:click="submitField">{{ t("common.save") }}</AppButton>
        </div>
    </AppModal>
    ...
    <AppModal :show="showDeleteConfirm" max-width="sm" v-on:close="showDeleteConfirm = false">
        ...
        <div class="flex justify-end gap-2 pt-2 border-t border-line">
            <AppButton variant="ghost" size="md" v-on:click="showDeleteConfirm = false">{{ t("common.cancel") }}</AppButton>
    ...
    <AppModal :show="!!viewingSubmission" max-width="md" v-on:close="viewingSubmission = null">
        ...
        <div class="flex justify-end pt-2 border-t border-line">
            <AppButton variant="ghost" size="md" v-on:click="viewingSubmission = null">{{ t("common.cancel") }}</AppButton>
        </div>
    </AppModal>
</template>
