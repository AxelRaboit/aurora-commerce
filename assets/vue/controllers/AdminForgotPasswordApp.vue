...
<template>
    ...
    <template v-if="!status">
        <p class="mb-4 text-sm text-secondary">{{ t('auth.forgot_password.instructions') }}</p>
        ...
        <form method="POST" :action="submitPath" class="space-y-4" v-on:submit.prevent="handleSubmit">
            <AppInput
                ...
                required
            />
            <AppButton type="submit" class="w-full">{{ t('auth.forgot_password.submit') }}</AppButton>
        </form>
    </template>
    ...
    <div class="mt-4 text-center">
        <AppLink :href="loginPath" class="text-sm">{{ t('auth.forgot_password.back_login') }}</AppLink>
    </div>
</template>
