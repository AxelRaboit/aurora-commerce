            ...
            <div class="hidden sm:block bg-surface border border-line/60 rounded-xl overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-surface-2 border-b border-line">
                        <tr>
                            <th class="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.parameters.key') }}</th>
                            <th class="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.parameters.value') }}</th>
                            <th class="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted hidden md:table-cell">{{ t('admin.parameters.description') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-line">
                        ...
                        <tr v-if="!parameters.parsedParameters.value.items?.length">
                            <td colspan="3" class="px-4 py-8 text-center text-sm text-muted">{{ t('admin.parameters.empty') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-if="props.tab === 'users'" class="space-y-4">
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <div class="flex items-center gap-1.5">
                                ...
                                <span v-if="user.isCurrent" class="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-indigo-500/15 text-indigo-400 shrink-0">{{ t('admin.users.you') }}</span>
                            </div>
            ...
            <div class="hidden sm:block bg-surface border border-line rounded-lg overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-surface-2 border-b border-line">
                        <tr>
                            <th class="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.users.name') }}</th>
                            <th class="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.users.email') }}</th>
                            <th class="text-left px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted hidden md:table-cell">{{ t('admin.users.created') }}</th>
                            <th class="text-right px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.users.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-line">
                        <tr v-for="user in users.parsedUsers.value.items" :key="user.id" class="hover:bg-surface-2/50 transition-colors">
                            <td class="px-4 py-3 font-medium text-primary">
                                {{ user.name }}
                                <span v-if="user.isCurrent" class="ml-1 text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-indigo-500/15 text-indigo-400">{{ t('admin.users.you') }}</span>
                            </td>
                        ...
                        <tr v-if="!users.parsedUsers.value.items?.length">
                            <td colspan="4" class="px-4 py-8 text-center text-sm text-muted">{{ t('admin.users.empty') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-if="props.tab === 'access_requests'" class="space-y-4">
            ...
            <div class="hidden sm:block bg-surface border border-line rounded-lg overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-surface-2 border-b border-line">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.access_requests.requester') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-muted hidden md:table-cell">{{ t('admin.access_requests.message') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.access_requests.status') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-muted hidden lg:table-cell">{{ t('admin.access_requests.date') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-muted hidden lg:table-cell">{{ t('admin.access_requests.expires') }}</th>
                            <th class="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-muted">{{ t('admin.users.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-line">
                        ...
                        <tr v-if="!accessRequests.parsedAccessRequests.value.items?.length">
                            <td colspan="6" class="px-4 py-8 text-center text-sm text-muted">{{ t('admin.access_requests.empty') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
