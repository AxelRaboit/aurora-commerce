...
describe("AppColorPicker", () => {
    it("renders 16 preset color buttons by default", () => {
        ...
        const buttons = wrapper.findAll("button[type='button']").filter(
            (b) => b.attributes("title")?.startsWith("#"),
        );
    ...
    it("marks the active preset with border-primary class", () => {
        ...
        const wrapper = mount(AppColorPicker, { props: { modelValue: activeColor } });
        const activeButton = wrapper.findAll("button[type='button']").find(
            (b) => b.attributes("title") === activeColor,
        );
        expect(activeButton?.classes()).toContain("border-primary");
    });

    it("non-active presets do not have border-primary class", () => {
        const wrapper = mount(AppColorPicker, { props: { modelValue: "#3b82f6" } });
        const inactiveButton = wrapper.findAll("button[type='button']").find(
            (b) => b.attributes("title") === "#ef4444",
        );
    ...
    it("emits update:modelValue when a preset is clicked", async () => {
        const wrapper = mount(AppColorPicker, { props: { modelValue: null } });
        const firstPreset = wrapper.findAll("button[type='button']").find(
            (b) => b.attributes("title")?.startsWith("#"),
        );
    ...
    it("displays error message when error prop is set", () => {
        ...
        expect(wrapper.find("p.text-red-500").text()).toBe("Please pick a color");
    });
});
