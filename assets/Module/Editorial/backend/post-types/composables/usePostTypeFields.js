...
const EMPTY_FIELD = {
    ...
    translatable: false, choicesText: "", referencePostTypeId: null, referenceMultiple: false,
};
...
function fieldFromEntity(f) {
    ...
    return {
        ...
        required: f.required, translatable: f.translatable,
...
function buildFieldOptions(form) {
    ...
    if (form.type === PostFieldType.Reference) {
        ...
        if (form.referencePostTypeId) options.postTypeId = Number(form.referencePostTypeId);
        return options;
    }
    return {};
}
...
export function usePostTypeFields(props, selected, replacePostType) {
    ...
    const { modal: fieldModal, form: fieldForm, openCreate, openEdit, submit: submitField } = useFormModal({
        empty:      () => ({ ...EMPTY_FIELD }),
        fromEntity: fieldFromEntity,
        createUrl:  () => buildPath(props.fieldCreatePath, { id: selected.value.id }),
        editUrl:    (field) => buildPath(props.fieldEditPath, { id: selected.value.id, fieldId: field.id }),
        buildBody:  (form) => ({
            name: form.name, label: form.label, type: form.type,
            required: form.required, translatable: form.translatable,
            options: buildFieldOptions(form),
        }),
        onSuccess: ({ data }) => replacePostType(data.postType),
    });

    function openCreateField() { if (selected.value) openCreate(); }
    function openEditField(field) { openEdit(field); }
    ...
    async function confirmDeleteField() {
        ...
        try {
            ...
            if (!data?.success) { toast.error(t("shared.common.error")); return; }
    ...
    watch(
        ...
        (fields) => { orderedFields.value = [...(fields ?? [])].sort((a, b) => a.position - b.position); },
    ...
    return {
        fieldModal, fieldForm,
        openCreateField, openEditField, submitField,
        deletingField, confirmDeleteField,
        orderedFields, persistFieldOrder,
    };
}
