...
<template>
    <div class="flex flex-col gap-2 px-1">
        ...
        >
            ...
            <span class="text-sm text-muted">{{ uploading ? t("shared.common.loading") : t("admin.posts.addImage") }}</span>
            <input
                ref="inputRef"
                type="file"
                accept="image/*"
        ...
        <Teleport to="body">
            ...
            >
                <div v-if="previewOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" v-on:click="previewOpen = false">
                    <img :src="mediaUrl" class="max-w-[90vw] max-h-[90vh] object-contain rounded-lg shadow-2xl" :alt="t('admin.posts.featuredImage')">
                </div>
            </Transition>
        </Teleport>
    </div>
