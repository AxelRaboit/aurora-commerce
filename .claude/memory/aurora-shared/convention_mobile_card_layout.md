...
- Les filtres côté serveur (liste paginée via `useListPage`) : `useListPage` + `extraParams`.

## Pourquoi

Cohérence responsive sur toutes les pages CRUD admin. Le pattern mobile card + tableau desktop est la norme Aurora pour toutes les listes.

## Comment l'appliquer

Dès qu'on ajoute ou retouche une page de liste CRUD, vérifier que le pattern mobile card est présent. Si absent, l'ajouter dans le même commit.
