...
| 2026-05-30 | **ModuleParameterEnum extensible** | 🚧 **PARTIEL** — la **cascade runtime** est faite (registry-driven : `ModuleToggleRegistry::getDescendantKeys()` + `SettingsService` ne dépend plus de l'enum). **Reste** : `CoreModuleParameterProvider` (registre les cases comme settings), `ModulesViewBuilder` + `UsersViewBuilder` (itèrent encore `::cases()`), puis distribution des 74 cases vers des enums par-module + Contexts. **Surface design** : le dashboard groupe par **parent structurel** (`getParentCase`) alors que `ModuleToggle.parentKey` = cascade-requires → il faudra ajouter un champ « parent structurel » à `ModuleToggle`. Gros refacto dédié, à faire à froid. NB : le ref module→`ModuleParameterEnum` (core) est **autorisé par l'invariant** — c'est du polish pour un `aurora-core` pristine, pas un blocker fonctionnel. |
