...
| 2026-05-30 | **J6 — Rollout : 11 repos splittés + poussés** | ✅ **FAIT**. `bin/split-modules.sh` a splitté + poussé les 11 packages restants vers leurs repos GitHub : aurora-{crm,billing,editorial,photo,project,hr,notes,personal-finance,planning,assistant} (single-prefix, historique préservé) + **aurora-commerce** (Ecommerce+Erp combinés sous `Ecommerce/`+`Erp/` + composer.json racine, snapshot). Avec aurora-tools (déjà fait) + aurora-core (le monorepo) = **les 13 packages existent sur GitHub**. Reste hors-code : Packagist/publish + adoption par client (câblage VCS+bundles+routes, cf. ce qu'on a fait pour Tools) + méta-package de transition. |
