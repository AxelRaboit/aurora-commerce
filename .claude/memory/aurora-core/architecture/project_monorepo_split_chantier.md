...
| 2026-05-30 | **ModuleParameterEnum extensible** | ✅ **MÉCANISME FAIT** / 🚧 distribution cases = follow-up. **Tous les consommateurs sont registry-driven** : `SettingsService` (cascade via `ModuleToggleRegistry::getDescendantKeys()`), `ModulesViewBuilder` + `UsersViewBuilder` (plus aucun `::cases()`). `ModuleToggle` a gagné `displayParentKey` (parent structurel dashboard, distinct de `parentKey`=cascade) + registry `getDisplayTopLevel()`/`getDisplayChildrenOf()`. `getCascadeDisableTargets()` est mort. **Reste (mécanique, batch)** : distribuer les 74 cases vers des enums par-module + `CoreModuleParameterProvider`→providers par-module + Contexts. ⚠️ **À faire en une fois** : une distribution partielle (qq modules sortis, le reste dans l'enum central) est un état plus sale qu'un enum entier. NB : le ref module→`ModuleParameterEnum` (core) est **autorisé par l'invariant** — polish pour `aurora-core` pristine, pas un blocker. |
