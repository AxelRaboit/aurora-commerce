...
| — | J5b — Exécution rollout (12 autres packages) | 🔜 prérequis **infra** (hors-code) : `splitsh-lite` + repos GitHub enfants + **Gate 2 assets Vue** (toujours ouvert) pour les packages avec front. Le code est dé-risqué : appliquer le template Tools (composer.json + services.php) à chaque module et valider in-monorepo. |
