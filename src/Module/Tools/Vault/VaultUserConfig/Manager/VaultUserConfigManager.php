...
namespace Aurora\Module\Tools\Vault\VaultUserConfig\Manager;
...
use Aurora\Module\Tools\Vault\VaultUserConfig\Dto\VaultUserConfigInputInterface;
...
class VaultUserConfigManager implements VaultUserConfigManagerInterface
{
    ...
    public function destroyVault(CoreUserInterface $user, VaultUserConfigInterface $config): void
    {
        ...
        $this->entityManager->createQuery('DELETE FROM Aurora\Module\Tools\Vault\VaultEntry\Entity\VaultEntry e WHERE e.user = :user')
            ->setParameter('user', $user)
            ->execute();

        $this->entityManager->createQuery('DELETE FROM Aurora\Module\Tools\Vault\VaultFolder\Entity\VaultFolder f WHERE f.user = :user')
            ->setParameter('user', $user)
            ->execute();
