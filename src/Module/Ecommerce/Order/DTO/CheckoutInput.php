...
final readonly class CheckoutInput
{
    public function __construct(
        #[Assert\NotBlank(message: 'ecommerce.checkout.errors.email_required')]
        #[Assert\Email(message: 'ecommerce.checkout.errors.email_invalid')]
        public string $email = '',
        #[Assert\NotBlank(message: 'ecommerce.checkout.errors.name_required')]
    ...
    public function shippingErrors(): array
    {
        ...
        if (null === $this->addressLine1 || '' === $this->addressLine1) {
            $errors['addressLine1'] = 'ecommerce.checkout.errors.address_required';
        }

        if (null === $this->city || '' === $this->city) {
            $errors['city'] = 'ecommerce.checkout.errors.city_required';
        }

        if (null === $this->postalCode || '' === $this->postalCode) {
            $errors['postalCode'] = 'ecommerce.checkout.errors.postal_required';
        }

        if (null === $this->country || '' === $this->country) {
            $errors['country'] = 'ecommerce.checkout.errors.country_required';
        } elseif (!in_array($this->country, CountryEnum::values(), true)) {
            $errors['country'] = 'ecommerce.checkout.errors.country_invalid';
        }
