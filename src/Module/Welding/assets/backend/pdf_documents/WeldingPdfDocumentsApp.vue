...
<template>
    <div class="space-y-4">
        <AppListToolbar>
            <AppSearchInput v-model="searchInput" :placeholder="t('backend.welding.pdf_documents.searchPlaceholder')" v-on:search="onSearch" />
            <template #actions>
                <AppButton
                    v-if="can('welding.pdf_documents.generate')"
                    variant="primary"
                    size="md"
                    class="w-full sm:w-auto"
                    v-on:click="openModal"
                >
                    <Plus class="w-4 h-4" :stroke-width="2" /> {{ t("backend.welding.pdf_documents.add") }}
                </AppButton>
            </template>
        </AppListToolbar>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-surface-2/50 border-b border-line/40">
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted">{{ t("backend.welding.pdf_documents.reference") }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted hidden md:table-cell">{{ t("backend.welding.pdf_documents.template") }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted hidden lg:table-cell">{{ t("backend.welding.pdf_documents.status") }}</th>
                        ...
                        <tr v-for="doc in items" :key="doc.id" class="group hover:bg-surface-2/40 transition-colors">
                            ...
                            <td class="px-6 py-3">
                                <div class="flex items-center justify-end gap-0.5">
                                    <AppIconButton v-if="doc.downloadUrl" color="sky" :title="t('backend.welding.pdf_documents.view')" v-on:click="viewingDoc = doc"><Eye class="w-4 h-4" :stroke-width="2" /></AppIconButton>
                                    <a v-if="doc.downloadUrl" :href="doc.downloadUrl" target="_blank" rel="noopener">
                                        <AppIconButton color="sky" :title="t('backend.welding.pdf_documents.download')"><Download class="w-4 h-4" :stroke-width="2" /></AppIconButton>
                                    </a>
                                    <AppIconButton v-if="can('welding.pdf_documents.delete')" color="rose" :title="t('shared.common.delete')" v-on:click="confirmDelete(doc)"><Trash2 class="w-4 h-4" :stroke-width="2" /></AppIconButton>
                                </div>
                            </td>
                        </tr>
                        <tr v-if="!items?.length">
                            <td :colspan="4"><AppNoData :message="t('backend.welding.pdf_documents.empty')" /></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <AppModal
            :show="showModal"
            :title="step === 1 ? t('backend.welding.pdf_documents.pickTemplate') : step === 3 ? t('backend.welding.pdf_documents.signatureTitle') : (editorTemplate?.name ?? '')"
        ...
        >
            ...
            <div v-if="step === 1" class="space-y-3">
                <AppSearchInput :placeholder="t('backend.welding.pdf_templates.searchPlaceholder')" v-on:search="debouncedSearch" />
                <div class="min-h-48 max-h-96 overflow-y-auto scrollbar-thin rounded-lg border border-line">
                    ...
                    <div v-else-if="!pickerItems.length" class="px-4">
                        <AppNoData :message="t('backend.welding.pdf_documents.noActiveTemplate')" />
                    </div>
                    <template v-else>
            ...
            <div v-else-if="step === 3" class="space-y-4">
                <p class="text-sm text-secondary">{{ t("backend.welding.pdf_documents.signatureDesc") }}</p>
            ...
            <div v-else class="flex gap-6 h-[70vh]">
                <div class="hidden lg:flex flex-col flex-1 min-w-0 overflow-hidden">
                    ...
                    <div v-else class="flex-1 flex items-center justify-center text-muted text-sm rounded-lg border border-line bg-surface-2/40">
                        {{ t("backend.welding.pdf_documents.noPreview") }}
                    </div>
                </div>
                <div class="flex flex-col w-full lg:w-80 shrink-0 overflow-y-auto scrollbar-thin space-y-4 pr-1">
                    <AppInput v-model="generateForm.label" :label="t('backend.welding.pdf_documents.label')" :placeholder="t('backend.welding.pdf_documents.labelPlaceholder')" />
                    ...
                    <div v-if="editorTemplate?.fields?.length" class="space-y-3">
                        <p class="text-xs font-medium uppercase tracking-wider text-muted">{{ t("backend.welding.pdf_documents.fieldValues") }}</p>
            ...
            <template #footer>
                <AppModalFooter>
                    ...
                    <template v-else-if="step === 3">
                        ...
                        >
                            <FileOutput class="w-3.5 h-3.5" :stroke-width="2" /> {{ t("backend.welding.pdf_documents.generate") }}
                        </AppButton>
                    </template>
                    <template v-else>
                        ...
                        <AppButton v-if="editorTemplate?.requiresSignature" variant="primary" size="md" v-on:click="goToSignature">
                            <PenLine class="w-3.5 h-3.5" :stroke-width="2" /> {{ t("backend.welding.pdf_documents.signatureNext") }}
                        </AppButton>
                        <AppButton
                            v-else
                            variant="primary"
            ...
            <template #footer>
                <AppModalFooter>
                    ...
                    <a :href="viewingDoc?.downloadUrl" target="_blank" rel="noopener">
                        <AppButton variant="secondary" size="md"><Download class="w-3.5 h-3.5" :stroke-width="2" /> {{ t("backend.welding.pdf_documents.download") }}</AppButton>
                    </a>
                </AppModalFooter>
            </template>
        </AppModal>
        ...
        >
            <p class="text-sm text-primary">{{ t("backend.welding.pdf_documents.deleteConfirm", { reference: pendingDelete?.reference ?? "" }) }}</p>
            <p class="text-sm text-secondary">{{ t("backend.welding.pdf_documents.deleteWarning") }}</p>
            <template #footer>
                <AppModalFooter>
