...
describe("useClientFilteredList", () => {
    ...
    it("returns all items when search is empty", () => {
        const { filteredItems } = useClientFilteredList(
            items,
            "/list",
            matcher,
        );
    ...
    it("filters by matcher predicate (case-insensitive trimmed)", async () => {
        const { searchInput, filteredItems } = useClientFilteredList(
            items,
            "/list",
            matcher,
        );
    ...
    it("reload() refetches from listPath and replaces items", async () => {
        global.fetch.mockResolvedValueOnce({
            json: async () => ({
                items: [{ id: 99, label: "Reloaded", slug: "reloaded" }],
            }),
        });
        const { filteredItems, reload } = useClientFilteredList(
            items,
            "/list",
            matcher,
        );
    ...
    it("reload() defaults to empty array when payload has no items key", async () => {
        ...
        const { items: itemsRef, reload } = useClientFilteredList(
            items,
            "/list",
            matcher,
        );
    ...
    it("does not mutate the initial array reference", () => {
        const seed = [...items];
        const { items: itemsRef } = useClientFilteredList(
            seed,
            "/list",
            matcher,
        );
